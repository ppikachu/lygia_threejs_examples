...
void main()
{
  ...
  float rainAmount = sin(u_time * 0.5) * 0.3 + 0.7;
  ...
  float fade = smoothstep(0., 10., u_time);                           // fade in at the start
  ...
  if (onoff) gl_FragColor = vec4(col, 1.);
  else gl_FragColor = texture2D(u_tex0, vTexCoordinate);
}